...
const GMOFX_WS_URL: &str = "wss://forex-api.coin.z.com/ws/public/v1";
...
pub async fn connect(
    ...
    let request = GMOFX_WS_URL.into_client_request()?;
    let (ws_stream, _) = connect_async(request).await?;
    let (mut write, mut read) = ws_stream.split();

    // GMO FX enforces a rate limit on subscribe commands.
