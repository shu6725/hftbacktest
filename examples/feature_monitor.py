#!/usr/bin/env python3
"""
GMO BTC_JPY 5-second bar feature monitor.
...
Display updates every 5 s on bar close:
  - Price panel   : GMO bid/ask, FX rate, CB×FX derived price, basis
  - Bar panel     : OHLCV, OFI, trade count rate
  - Feature panel : all 37 features (microstructure + TA-Lib)

Usage:
  LD_LIBRARY_PATH=$HOME/.local/lib python3 examples/feature_monitor.py
"""
...
try:
    ...
    _TALIB = False
    print("[warn] ta-lib not available; TA-Lib features will show N/A", file=sys.stderr)
...
def _render() -> str:
    ...
    lines = [_top(f"GMO BTC/JPY Monitor  {now}")]
    ...
    if bid_u and ask_u and bid_c and ask_c:
        der_bid = bid_c * bid_u; der_ask = ask_c * ask_u
        lines.append(_box(f"  CB×FX BTC/JPY bid {_fp(der_bid,0):>14}  ask {_fp(der_ask,0):>14}"))
        if bid_g and ask_g:
            gmo_mid = (bid_g + ask_g) / 2; der_mid = (der_bid + der_ask) / 2
            basis = (gmo_mid / der_mid - 1) * 10_000
            lines.append(_box(f"  Basis (GMO/CB×FX)              {basis:>+10.2f} bps"))
    else:
        lines.append(_box(f"  CB×FX BTC/JPY                       N/A"))
    ...
    if b2:
        ...
        if b2["impact_spread_bps"] is not None:
            lines.append(_box(f"  impact_spread_bps {b2['impact_spread_bps']:>+10.4f}"))
    else:
        lines.append(_box("BAR  (waiting for first bar…)"))

    # ── Features ─────────────────────────────────────────────────────────────
    lines.append(_sep())
    need = max(ROLL_N, 34 if _TALIB else ROLL_N)
    nb = len(_bars)
    lines.append(_box(f"FEATURES  (bars={nb}, need≥{need} for all TA-Lib)"))
    lines.append(_box(f"{'Feature':<30} {'Value':>20}"))
    lines.append(_box(f"{'─'*30} {'─'*20}"))
    for name, dec in FEATURE_ORDER:
        v = _last_features.get(name)
        lines.append(_frow(name, v, dec))
